...
function set_vars()
{
    if [ $ROW_NUM -le ${TOT_ROWS} ]; then
        ...
        MACRO_THETA_ROW=`awk '/\/WGR16\/generator\/theta/{ print NR; exit }' ${MACRO}`
        ...
        if [ $BATCH_SYS = "HTCondor" ]; then
            ...
            sed -i "${TRANSFER_INPUT_ROW}s?=.*?= ${CEPC_BUILD_DIR}/CMakeFiles, ${CEPC_BUILD_DIR}/CMakeCache.txt, ${CEPC_BUILD_DIR}/HTCondor_env.sh, ${CEPC_BUILD_DIR}/condor_exe.sh, ${CEPC_BUILD_DIR}/init.mac, ${CEPC_BUILD_DIR}/run_ele.mac, ${CEPC_BUILD_DIR}/run_pi.mac, ${CEPC_BUILD_DIR}/WGR16?" condor.sub
            
            #SOURCE_ROW=`awk '/source/{ print NR; exit }' condor_exe.sh`
            #sed -i "${SOURCE_ROW}s?.*?source ${CEPC_BUILD_DIR}/HTCondor_env.sh?" condor_exe.sh
            
            EXE_ROW=`awk '/WGR16/{ print NR; exit }' condor_exe.sh`
            sed -i "${EXE_ROW}s?.*?\./WGR16 ${MACRO} \$1?" condor_exe.sh
            ...
            EXE_ROW=`awk '/WGR16/{ print NR; exit }' torque_exe.sh`
            sed -i "${EXE_ROW}s?.*?${CEPC_BUILD_DIR}/WGR16 ${CEPC_BUILD_DIR}/${MACRO} \$1?" torque_exe.sh
